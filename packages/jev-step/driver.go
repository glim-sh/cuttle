...
func drive(ctx context.Context, args ...string) (string, error) {
	out, err := exec.CommandContext(ctx, playwrightCLI, args...).CombinedOutput()
	if err != nil {
...
func execute(ctx context.Context, dec decision, step planStep) error {
	...
	case actionClick:
		...
		return err
	case actionType:
		value, ok := step.Fill[dec.Value]
		if !ok {
			return fmt.Errorf("%w: chose fill value %q, which the plan step does not define", errDriver, dec.Value)
		}
