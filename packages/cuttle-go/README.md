...
The Go implementation of `cuttle` - a stealth-Chromium CDP farm and a universal
...
> This package is the in-progress Go rewrite. The Python oracle in
> [`../cuttle-py`](../cuttle-py) remains the shipped implementation until the Go
> port reaches fingerprint/stealth parity. See
> [`docs/plans/2607-16-cuttle-go-rewrite.md`](../../docs/plans/2607-16-cuttle-go-rewrite.md).
...
go install github.com/glim-sh/cuttle/packages/cuttle-go/cmd/cuttle@latest
```
...
parity-tested byte-for-byte against the Python oracle - the goldens are
regenerated with `just parity-golden`. The Dockerfile is Python-free: a static Go
binary plus the verbatim clark/clearcote engine and KasmVNC/noVNC stages.
