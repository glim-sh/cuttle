...
Requires ``geoip2`` and ``socksio`` (both are cuttle runtime dependencies).
Downloads GeoLite2-City.mmdb (~70 MB) on first use, caches under the cache dir.
Background re-download after 30 days.
"""
...
def resolve_proxy_geo_with_ip(
    ...
    try:
        ...
        raise ImportError("geoip2 is required for geoip=True") from None
...
def _resolve_exit_ip(proxy_url: str | None, timeout: float | None = None) -> str | None:
    ...
    for url in _IP_ECHO_URLS:
        try:
            ...
            logger.warning(
                "SOCKS5 proxy requires the socksio package"
            )
            return None
        except Exception:
            continue
